...
class MinimalHoloGen(nn.Module):
    ...
    def forward(self, condition_ids, resonance_steps=12):
        ...
        # We now blend the initial state with the persistent standing wave (Resonant Injection)
        # This makes the "long-lived" memory the substrate for the new resonance
        initial_excitation = self.condition_embed(condition_ids)
        state = 0.8 * initial_excitation + 0.2 * self.standing_wave[condition_ids]
        
        memory = state.clone()
        phi_t = torch.zeros(batch, self.field_size, device=device)

        # Accumulate states for peak detection (finding constructive interference)
        all_states = []
        ...
        for _ in range(resonance_steps):
            ...
            state = state + E * coherence * 0.08   # measurement feedback
        ...
        # Peak Detection - finding the highest energy state (constructive interference)
        # This is more "honest" for a wave system than simple averaging
        peak_wave = torch.stack(all_states, dim=1).max(dim=1)[0]

        # Update the Persistent standing wave
        if self.training:
            with torch.no_grad():
                # Blend the new peaks into the long-lived pool
                for i in range(batch):
                    cid = condition_ids[i]
                    self.standing_wave[cid] = (1 - self.pool_rate) * self.standing_wave[cid] + self.pool_rate * peak_wave[i]

        # Final readout from the peak-detected state
        final_state = peak_wave
...
def train():
    ...
    print(f"HOLOGRAPHIC CIFAR TRAINING - Device: {device}")

    train_loader, test_loader, num_classes, img_size = get_dataloader('cifar', args.batch_size)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            plt.imsave(f"samples/cifar_epoch_{epoch+1:02d}.png", grid.permute(1, 2, 0).cpu().numpy())

    torch.save(model.state_dict(), f"checkpoints/holo_cifar_final.pth")
