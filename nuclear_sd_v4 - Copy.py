...
def run_autopilot(n_steps: int = 400):
    ...
    for k, v in CLASSIFICATION_LABELS.items():
        bar = "#" * cls_counts[k]
        print(f"    {_col(k)}[{k}]{RESET}  {v:<26}  {cls_counts[k]:3d}  {bar}")

    anomalies = sorted(
        [r for r in all_results if r["gradient_anomaly"] > 0.05],
        key=lambda x: -x["gradient_anomaly"])
    if anomalies:
        print(f"\n  Gradient anomalies ((A) dynamics exceed prediction):")
        print(f"  {'Cls':<4} {'Isotope':<42} {'anomaly':>9} {'BE dev':>8} {'shell':>6} {'Dv':>8}")
        for r in anomalies:
            cfg = r["config"]
...
def run_sweep(z_range, n_range, n_steps=300, output_file="landscape.csv", label="sweep"):
    total = sum(1 for Z in z_range for N in n_range if not (Z > 2 and N < Z * 0.4))
...
def run_gradient_report(n_steps: int = 400):
    print("\n" + "=" * 120)
    ...
    print(f"\n  {'Cls':<4} {'Isotope':<45} {'anomaly':>9} {'BE dev':>8} "
          f"{'shell':>6} {'Δvalley':>8} {'decay rate':>11} {'N/Z':>6}")
    print("  " + "─" * 110)
    for r in results:
        cfg  = r["config"]
        flag = " ◀" if r["gradient_anomaly"] > 0.05 else ""
        print(f"  {_col(r['classification'])}[{r['classification']}]{RESET}  "
              f"{cfg.symbol:<43}  {r['gradient_anomaly']:>+9.3f}  "
              f"{r['binding_deviation_mev']:>+7.3f}  "
              f"{r['shell_proximity']:>6.2f}  "
              f"{r['valley_displacement']:>+8.1f}  "
              f"{r['_decay_rate']:>11.5f}  "
              f"{r['nz_ratio']:>6.4f}{flag}")
    print("=" * 120 + "\n")
